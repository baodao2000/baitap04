...
export default defineComponent({
  ...
  setup() {
    ...
    const targetNetwork = 'Rinkeby test network'
    ...
    const connectWallet = async () => {
      ...
      try {
        ...
        const res = await fetch(`https://testnets-api.opensea.io/api/v1/assets?owner=${walletStore.acc_long}&order_direction=desc&offset=0&limit=20&include_orders=false`)
        const datas = await res.json()
        
        assetsData.value = datas  ;
        console.log("asset:",assetsData.value);
